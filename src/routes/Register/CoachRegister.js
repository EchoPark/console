...
import { createUserForCoach, checkLoginState, deleteUserInfo } from '../../services/register'
...
class CoachRegister extends Component {
  constructor (props) {
    super(props)
    this.state = {
      idPicUrl: '',
      drivingLicensePicFrontUrl: '',
      drivingLicensePicBackUrl: '',
      checked: false, // 用户注册协议是否打钩
      identifyingCode: '获取验证码', // 验证码按钮显示文字
      currentStep: 0, // 当前所在步骤
      checkStep: 2,  // 审核所在步骤
      idPic: '',  // 身份证图片
      drivingLicensePicFront: '', // 驾照图片正面
      drivingLicensePicBack: '',  // 驾照图片反面
      registerInfo: {},  // 用户注册信息
      verifyCode: '',    // 验证码
      registerMessage: '',   //
      errMsg: '',   // 错误信息提示
  ...
  getIdentifyingCode = (() => {
    let timeId = null
    let count = 60  // initialization
    let identifyingCode = '' // countdown
    return () => {
      if (timeId) {
        return
      }
    ...
    if (action === 'prev') {
      ...
      switch (currentStep) {
        case 1:
          ...
          break
        case 2:
          validate = this.inputValidate(['coach_no', 'school_name', 'address'])
          break
      }
      if (validate) {
        if (currentStep === 1) {
        ...
        if (!registerInfo[key]) {
          switch (key) {
            ...
            case 'phone':
              ...
              break
            case 'coach_no':
              this.setState({ errMsg: '教练证编号不能为空' })
              break
            case 'school_name':
              this.setState({ errMsg: '驾校不能为空' })
              break
            case 'address':
              this.setState({ errMsg: '地址不能为空' })
              break
          }
          return false
        } else {
  ...
  registerUser = async userInfo => {
    ...
    if (code) {
      const { data } = await createUserForCoach({ code, userInfo })
      return data.code
    }
  }
  // 检查注册状态
  ...
  // 提交表单
  submit = () => {
    const { registerInfo, idPicUrl, drivingLicensePicFrontUrl, drivingLicensePicBackUrl } = this.state
    Toast.loading(`提交中`, 0)
    this.registerUser({
      files: [
        {
        ...
        {
          name: 'coachfront',
          path: drivingLicensePicFrontUrl
        },
        {
          name: 'coachBack',
          path: drivingLicensePicBackUrl
        }
      ],
      ...registerInfo
    }).then(code => {
  ...
  render () {
    const { checked, identifyingCode, currentStep, checkStep, idPic, drivingLicensePicFront, drivingLicensePicBack } = this.state
    ...
    const step2 = (
      <div style={{ margin: '2rem 0.3rem 0' }}>
        <InputItem
          placeholder='教练证编号'
          onChange={this.inputChange.bind(this, 'coach_no')}
          value={registerInfo.coach_no ? registerInfo.coach_no : ''}
        />
        <InputItem
          placeholder='所属驾校'
          style={{ marginTop: '0.1rem' }}
        ...
        />
        <Button
          disabled={!(registerInfo.coach_no && registerInfo.school_name && registerInfo.address)}
          onClick={this.stepChange.bind(this, 'next')}
          type='primary'
          style={{ marginTop: '0.2rem' }}
        >
    ...
    const step3 = (
      <div style={{ margin: '2rem 0.3rem 0' }}>
        ...
        <ImgPicker
          imgSrc={idPic}
          addImg={this.chooseImage.bind(this, 'idPic')}
          cancelImg={this.cancelImage.bind(this, 'idPic')}
        />
        <p>上传教练证（正面）</p>
        <ImgPicker
          imgSrc={drivingLicensePicFront}
          addImg={this.chooseImage.bind(this, 'drivingLicensePicFront')}
          cancelImg={this.cancelImage.bind(this, 'drivingLicensePicFront')}
        />
        <p>上传教练证（反面）</p>
        <ImgPicker
          imgSrc={drivingLicensePicBack}
          addImg={this.chooseImage.bind(this, 'drivingLicensePicBack')}
          cancelImg={this.cancelImage.bind(this, 'drivingLicensePicBack')}
        />
        <Button
        ...
        <Button
          ...
          type='primary'
          style={{ marginTop: '0.2rem' }}
          disabled={!(drivingLicensePicFront && drivingLicensePicBack && idPic)}
        >
        提交注册
        </Button>
      </div>
